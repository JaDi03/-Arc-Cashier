...
They are often the same origin in dev. In production the browser usually hits a **plugin relay** on the platform origin (see Browser UI and relay).
...
- Platform has an admin **and** creators: use both. Admin: plugin settings dashboard. Creator: `initCreatorEarnings`.
- Platform is admin-only: use the admin one (plugin settings dashboard).
